...
public class AttributeHandler
{
	...
	{
		IAttributeInstance reachAttribute = player.getEntityAttribute( player.REACH_DISTANCE );
		float buildLevel = XP.levelAtXp( player.getEntityData().getCompoundTagTag( player.PERSISTED_NBT_TAG ).getCompoundTagTag( "skills" ).getFloat( "building" ) );
		if( buildLevel == 1 )
			return;
...
//			reach /= 2;
	...
	public static void updateSpeed( EntityPlayer player )
	{
		...
		float agilityLevel = XP.levelAtXp( player.getEntityData().getCompoundTagTag( player.PERSISTED_NBT_TAG ).getCompoundTagTag( "skills" ).getFloat( "agility" ) );
	...
	public static void updateHP( EntityPlayer player )
	{
		...
		float enduranceLevel = XP.levelAtXp( player.getEntityData().getCompoundTagTag( player.PERSISTED_NBT_TAG ).getCompoundTagTag( "skills" ).getFloat( "endurance" ) );
	...
	public static void updateDamage( EntityPlayer player )
	{
		...
		float combatLevel = XP.levelAtXp( player.getEntityData().getCompoundTagTag( player.PERSISTED_NBT_TAG ).getCompoundTagTag( "skills" ).getFloat( "combat" ) );
