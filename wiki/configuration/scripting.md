...
|   `mob_scale`   | the id of the mob being scaled                                                | keyword = `attribute`. the attributed id and scaling value separated by an operation symbol.  + = add, * = mulitply base, ** = multiply total. `minecraft:generic.max_health+1.001` for multiple attributes, continue the pattern `attribute(minecraft:generic.max_health+1.001,minecraft:generic.movement_speed*1.00001)` |
