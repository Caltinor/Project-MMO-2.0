...
Additionally, if you just want to make small edits, one at a time, the `/pmmo config` command will build you a datapack called "command_pack" one update at a time.

<Callout variant="info">the genData/config commands require Op permissions or cheats enabled.</Callout>

### Command options
`/pmmo config` lets you enter the settings you want in the command line and then creates a datapack using those settings for you behind the scenes.  Depending on the feature you are setting, many support tags and will copy your entered setting to each member of the tag.  Applicable options will appear in the command suggestions when as you type.
