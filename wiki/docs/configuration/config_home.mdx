...
Project MMO has three built-in methods for configuring the mod features.
1. Datapacks
2. Scripting
3. Server Configs

Additionally, there are two `.toml` config files in the `/config/` folder of your Minecraft installation.
- `pmmo-client.toml` contains settings for the client.  Settings related to visual aspects of pmmo are configured here and can be different for every client.
...
With the exception of NBT configs, both datapacks and scripting provide the same capabilities.  So it is purely your preference as to which you would like to use.  Both options are server-enforced, so whether you ship your settings to clients in a modpack or simply set them up on your server, the client does not need a copy for Project MMO to work and the server version is always the one used.
...
### SERVER Configs
There are six server configs which can be located at `./minecraft/saves/yoursave/serverconfigs/`.  The six configs are:
