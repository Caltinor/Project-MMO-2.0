...
# What Do Skills Do?
If you're playing the game and obtain experience in "Woodcutting", you might ask yourself "what does this skill do?" or "What can i do with this now?"  The answer is, it depends.  Skills are just containers for experience.  Some have perks (special abilities) configured for them, which increasing in levels makes better.  Some are used as requirements for using items or traveling to different biomes/dimensions.  And some are purely there to capture how much of a specific activity you have done.  It will purely depend on how your world is configured.  The glossary is invaluable and helping you find out what skills are
...
By default, skills display in white.  If that is okay with you, then you don't need to do anything here.  If you do want a custom color though, You will need to add your custom skill to the skills server config.  The color is a hex color in integer form.  if you know your hex color you can get the int value [HERE](https://www.programiz.com/javascript/online-compiler/) using the following code.  the output will be the number for your config.
```js
