...
## How do perks work
Perks are configured in the [perks config](../configuration/serverconfigs/perksconfig).  When an gameplay event occurs, requirements are checked first.  If the player is not prohibited from the action, special event triggers from addons occur.  After those, all perks configured for that event will attempt to activate.  Most perks have conditions such as specific skills, cooldowns, damage types, weapons used, etc.  For all that meet their conditions, they activate and their special abilities go into effect.  After this, any applicable experience is awarded.
...
### <u>pmmo:fireworks</u>
Shoots a firework into the air above the player.  Only works with the `SKILL_UP` or `SKILL_DOWN` event.
