...
|**SMELTED**|when an item is smelted in a vanilla furnace, the player who placed the furnace gets the experience|None|from the output item's data'|
...
|**SKILL_DOWN**| whenever a player's level increases.  this is not used to award XP but is used by Perks.|
