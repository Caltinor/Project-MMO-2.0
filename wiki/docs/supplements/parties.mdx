# Parties
parties allow players to join together to share XP gains.  Players need to be in proximity of each other, and those that are in range earn the same xp, plus a bonus, as their nearby party members.
...
use the command `/pmmo party list` to display all party members.  Parties are also stored in the server's scoreboard, so server operators can also configure the display of teams through this vanilla mechanic.

## Leave a Party
To leave a party use the command `/pmmo party leave`

## Party XP, How it works
Sharing xp always originates from the player who earned it.  party members who are in range of that player, and the player, get the same XP earned plus a percent.  For example, in the example below each "-" is a 10 block distance and our config is set to award XP to party members within 40 blocks.
```
B---A---C
```
if player "A" earns 30 XP, both B and C are in range everyone will receive 31 xp (assuming 5% bonus).  If player C earns 30 XP, only player A is in range, therefore players A and C get 31 XP each and nothing will be awarded to player B.