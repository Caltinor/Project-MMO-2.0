# Mob Scaling
Mob scaling is a feature that causes mobs spawned near players to have their attributes modified based on the skill level of those nearby players. All settings for mob scaling are located in the server config.  by default this feature is turned off.  The scaling can be difficult to balance and the monsters spawned with this system do not despawn which can lead to high level players griefing others by spawning strong mobs around weaker players.
