    ...
    public static void keyPressEvent( net.minecraftforge.client.event.InputEvent.KeyInputEvent event )
    {
        if( Minecraft.getInstance().player != null )
        {
            ...
            if( wasOpenMenu != ClientHandler.OPEN_MENU.isKeyDown() || wasOpenSettings != ClientHandler.OPEN_SETTINGS.isKeyDown() || wasOpenSkills != ClientHandler.OPEN_SKILLS.isKeyDown() || wasOpenGlossary != ClientHandler.OPEN_GLOSSARY.isKeyDown() )
            {
                ...
                String name = Minecraft.getInstance().player.getDisplayName().getUnformattedText();

                XP.playerNames.setTag( uuid, name );

                XPOverlayGUI.skills.forEach( (skill, aSkill) ->
                {
                    XP.getOfflineXpMap( uuid ).setTag( skill, aSkill.goalXp );
                });
                ...
                if( Minecraft.getInstance().currentScreen == null )
                {
                    ...
                    {
                        Minecraft.getInstance().displayGuiScreen( new PrefsChoiceScreen( new TextComponentTranslation( "pmmo.preferences" ) ) );
                    ...
                    {
                        Minecraft.getInstance().displayGuiScreen( new GlossaryScreen( uuid, new TextComponentTranslation( "pmmo.glossary" ), true ) );
            ...
            if( !(Minecraft.getInstance().player == null) && ClientHandler.TOGGLE_TOOLTIP.isKeyDown() && !tooltipKeyWasPressed )
            {
                ...
                if( TooltipHandler.tooltipOn )
                    Minecraft.getInstance().player.sendStatusMessage( new TextComponentTranslation( "pmmo.tooltipOn" ), true );
                else
                    Minecraft.getInstance().player.sendStatusMessage( new TextComponentTranslation( "pmmo.tooltipOff" ), true );
            }
