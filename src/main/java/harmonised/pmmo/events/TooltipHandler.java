...
//                }
                ...
                if( FConfig.getConfig( "autoGenerateValuesEnabled" ) != 0 )
                {
                    ...
                    if( FConfig.getConfig( "wearReqEnabled" ) != 0 && FConfig.getConfig( "autoGenerateWearReqDynamicallyEnabled" ) != 0 )
                    {
                        if( wearReq == null )
                            wearReq = new HashMap<>();
                        String wearReqSkill = FConfig.getConfig( "autoGenerateWearReqAsCombat" ) != 0 ? Skill.COMBAT.toString() : Skill.ENDURANCE.toString();
...
//                    tooltip.add( new TextComponentString( XP.checkMaterial( material ) + " " + XP.getSkill( material ) ) );
    ...
    private static void addTooltipTextSkill( String tKey, Map<String, Double> theMap, ItemTooltipEvent event )
    {
        ...
        if( theMap.size() > 0 )
        {
            tooltip.add( new TextComponentTranslation( tKey ).setStyle( XP.textStyle.get( XP.checkReq( player, theMap ) ? "green" : "red" ) ).getFormattedText() );
        ...
        if( theMap.size() > 0 )
        {
            ...
            for( String key : theMap.keySet() )
            {
                ...
                if( value < 0 )
                    ...
                    tooltip.add( new TextComponentTranslation( "pmmo.levelDisplayPercentage", " +" + DP.dp( value ), new TextComponentTranslation( "pmmo." + key ).getUnformattedText() ).setStyle( Skill.getSkillStyle( key ) ).getFormattedText() );
            }
        }
    }
}