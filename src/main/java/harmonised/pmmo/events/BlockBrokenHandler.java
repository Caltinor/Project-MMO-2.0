...
//            }
        ...
        if( XP.getExtraChance( player.getUniqueID(), block.getRegistryName(), JType.INFO_ORE, false ) > 0 )
        {
            ...
            if( !wasPlaced && !isSilk )
                award = XP.addMapsAnyDouble( award, XP.multiplyMapAnyDouble( XP.getXp( block.getRegistryName(), JType.XP_VALUE_BREAK ), theDropItem.getCount() ) );
            ...
            if( dropsItself && !wasPlaced || !dropsItself && !isSilk )			//EXTRA DROPS
            {
                ...
                award = XP.addMapsAnyDouble( award, XP.multiplyMapAnyDouble( XP.getXp( block.getRegistryName(), JType.XP_VALUE_BREAK ), totalExtraDrops ) );
            }
