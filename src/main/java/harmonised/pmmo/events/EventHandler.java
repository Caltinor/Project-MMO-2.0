...
public class EventHandler {
	...
	public static void blockPlaced(BlockEvent.EntityMultiPlaceEvent event) {
		if (event.getEntity() != null && BlockPlacedHandler.handlePlaced(event.getEntity(), event.getPlacedBlock(),
				(World) event.getWorld(), event.getPos()))
	...
	public static void travelDimension(EntityTravelToDimensionEvent event) {
		if (event.getEntity() instanceof ServerPlayerEntity) {
			...
			if (!XP.checkReq(player, reqMap)) {
				event.setCanceled(true);
				player.sendStatusMessage(new TranslationTextComponent("pmmo.notSkilledEnoughToTravelToDimension",
						new TranslationTextComponent(event.getDimension().getLocation().toString()))
