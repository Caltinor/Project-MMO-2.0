...
public class BreakHandler {
	...
	public static void handle(BlockEvent.BreakEvent event) {
		...
		if (core.getMarkedPos(event.getPlayer().getUUID()).equals(event.getPos())) {
			...
			if ((!block.requiresCorrectToolForDrops() || tool.isCorrectToolForDrops(block)) && !Config.VEIN_BLACKLIST.get().contains(RegistryUtil.getId(tool).toString())) {
	...
	private static Map<String, Long> calculateXpAward(Core core, BlockEvent.BreakEvent event, CompoundTag dataIn) {
		Map<String, Long> outMap = core.getExperienceAwards(EventType.BLOCK_BREAK, event.getPos(), (Level)event.getWorld(), event.getPlayer(), dataIn); 
		LevelChunk chunk = (LevelChunk) event.getWorld().getChunk(event.getPos());
