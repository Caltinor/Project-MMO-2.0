...
//
	...
	private void doXpDrops( MatrixStack stack )
	{
		...
		for( int i = 0; i < xpDrops.size(); i++ )		//update Xp Drops
		{
			...
			if( showXpDrops )
			{
				...
				if( tempAlpha > 3 )
					drawCenteredString( stack, fontRenderer, "+" + DP.dprefix( xpDrop.gainedXp ) + " " + new TranslationTextComponent( "pmmo." + xpDrop.skill.toLowerCase() ).getString(), xpDropPosX + (barWidth / 2), (int) xpDrop.Y + (int) xpDropOffset + xpDropPosY, (tempAlpha << 24) |+ Skill.getSkillColor( xpDrop.skill ) );
			}
		}
	...
	private void doXpBar()
	{
		...
		if( cooldown > 0 )				//Xp Bar
		{
			...
			if( aSkill.pos >= maxLevel )
				drawCenteredString( stack, fontRenderer, new TranslationTextComponent( "pmmo.levelDisplay", new TranslationTextComponent( "pmmo." + skill.toLowerCase() ).getString(), maxLevel ).getString(), barPosX + (barWidth / 2), barPosY, Skill.getSkillColor( skill ) );
			else
				drawCenteredString( stack, fontRenderer, new TranslationTextComponent( "pmmo.levelDisplay", new TranslationTextComponent( "pmmo." + skill.toLowerCase() ).getString(), DP.dp( Math.floor( aSkill.pos * 100D ) / 100D ) ).getString(), barPosX + (barWidth / 2), barPosY, Skill.getSkillColor( skill ) );

			if( (barKey || xpLeftDisplayAlwaysOn) && skills.get( skill ) != null )
			{
				if( skills.get( skill ).xp >= maxXp )
					drawCenteredString( stack, fontRenderer, new TranslationTextComponent( "pmmo.maxLevel" ).getString(), barPosX + (barWidth / 2), 17 + barPosY, Skill.getSkillColor( skill ) );
				else
				{
					if( goalXp >= maxXp )
						goalXp =  maxXp;
					...
					drawCenteredString( stack, fontRenderer, DP.dprefix( skills.get( skill ).xp ) + " / " + DP.dprefix( goalXp ), barPosX + (barWidth / 2), 17 + barPosY, Skill.getSkillColor( skill ) );
					drawCenteredString( stack, fontRenderer,  new TranslationTextComponent( "pmmo.xpLeft", DP.dprefix( goalXp - aSkill.xp ) ).getString(), barPosX + (barWidth / 2), 26 + barPosY, Skill.getSkillColor( skill ) );
				}
			}
...
//					multiplier = ( XP.getMultiplier(  player, tempSkill ) * 100 ) - 100;
			...
			skillsKeys.sort( Comparator.comparingDouble( a -> XP.getOfflineXp( (String) a, player.getUniqueID() ) ).reversed() );

			for( String keySkill : skillsKeys )
			{
				aSkill = skills.get( keySkill );
				skill = keySkill;
				level = XP.levelAtXpDecimal( aSkill.xp );
				tempString = DP.dp( Math.floor( level * 100D ) / 100D );
				color = Skill.getSkillColor( keySkill );
				if( level >= maxLevel )
					tempString = "" + maxLevel;
				...
				drawString( stack, fontRenderer, " | " + new TranslationTextComponent( "pmmo." + skill ).getString(), skillListX + levelGap + 4, skillListY + 3 + listIndex, color );
	...
	public static void sendLvlUp( int level, String skill )
	{
		...
		switch( skill )
		{
			...
			case "combat":
				...
				msg = new TranslationTextComponent( "pmmo.levelUpDamageBoost", level, new TranslationTextComponent( "pmmo." + skill.toLowerCase() ).getString(), DP.dpSoft( damageBoost ) );
				break;
