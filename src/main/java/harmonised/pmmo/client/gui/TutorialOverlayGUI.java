...
public class TutorialOverlayGUI implements IIngameOverlay {
    ...
    public void render(ForgeIngameGui gui, PoseStack stack, float partialTick, int screenWidth, int screenHeight) {
        ...
        if (!mc.options.renderDebug) {
            ...
            if (mc.level.getBlockState(bhr.getBlockPos()).getBlock()
                ...
                lines = new ArrayList<>(ctc(mc, LangProvider.SALVAGE_TUTORIAL_HEADER.asComponent()
            ...
            if (!lines.isEmpty()) {
                ...
                net.minecraftforge.client.event.RenderTooltipEvent.Color colorEvent = net.minecraftforge.client.ForgeHooksClient
                        .onRenderTooltipColor(ItemStack.EMPTY, stack, j2, k2, fontRenderer, lines);
                fillGradient(matrix4f, bufferbuilder, j2 - 3, k2 - 4, j2 + i + 3, k2 - 3, 400,
                        colorEvent.getBackgroundStart(), colorEvent.getBackgroundStart());
                fillGradient(matrix4f, bufferbuilder, j2 - 3, k2 + j + 3, j2 + i + 3, k2 + j + 4, 400,
                        colorEvent.getBackgroundEnd(), colorEvent.getBackgroundEnd());
                fillGradient(matrix4f, bufferbuilder, j2 - 3, k2 - 3, j2 + i + 3, k2 + j + 3, 400,
                        colorEvent.getBackgroundStart(), colorEvent.getBackgroundEnd());
                fillGradient(matrix4f, bufferbuilder, j2 - 4, k2 - 3, j2 - 3, k2 + j + 3, 400,
                        colorEvent.getBackgroundStart(), colorEvent.getBackgroundEnd());
                fillGradient(matrix4f, bufferbuilder, j2 + i + 3, k2 - 3, j2 + i + 4, k2 + j + 3, 400,
                        colorEvent.getBackgroundStart(), colorEvent.getBackgroundEnd());
                fillGradient(matrix4f, bufferbuilder, j2 - 3, k2 - 3 + 1, j2 - 3 + 1, k2 + j + 3 - 1, 400,
                        colorEvent.getBorderStart(), colorEvent.getBorderEnd());
                fillGradient(matrix4f, bufferbuilder, j2 + i + 2, k2 - 3 + 1, j2 + i + 3, k2 + j + 3 - 1, 400,
                        colorEvent.getBorderStart(), colorEvent.getBorderEnd());
                fillGradient(matrix4f, bufferbuilder, j2 - 3, k2 - 3, j2 + i + 3, k2 - 3 + 1, 400,
                        colorEvent.getBorderStart(), colorEvent.getBorderStart());
                fillGradient(matrix4f, bufferbuilder, j2 - 3, k2 + j + 2, j2 + i + 3, k2 + j + 3, 400,
    ...
    private List<MutableComponent> gatherSalvageData(ItemStack stack) {
        ...
        for (Map.Entry<ResourceLocation, SalvageData> entry : Core.get(LogicalSide.CLIENT).getLoader().ITEM_LOADER
            ...
            outList.add(new ItemStack(ForgeRegistries.ITEMS.getValue(entry.getKey())).getDisplayName().copy());
            ...
            if (!data.levelReq().isEmpty()) {
                ...
                for (Map.Entry<String, Integer> req : data.levelReq().entrySet()) {
                    outList.add(new TranslatableComponent("pmmo." + req.getKey())
                            .append(new TextComponent(": " + req.getValue())));
                }
            }
            ...
            if (!data.chancePerLevel().isEmpty()) {
                ...
                for (Map.Entry<String, Double> perLevel : data.chancePerLevel().entrySet()) {
                    outList.add(new TranslatableComponent("pmmo." + perLevel.getKey())
                            .append(new TextComponent(": " + perLevel.getValue())));
                }
            }
            if (!data.xpAward().isEmpty()) {
                ...
                for (Map.Entry<String, Long> award : data.xpAward().entrySet()) {
                    outList.add(new TranslatableComponent("pmmo." + award.getKey())
                            .append(new TextComponent(": " + award.getValue())));
                }
            }
        }
        return outList;
