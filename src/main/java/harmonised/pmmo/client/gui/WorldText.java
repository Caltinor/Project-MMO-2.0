   ...
   /* private static Map<PresetTextOption, CompoundTag> presetTextOptions = new HashMap<>();
    ...
    public boolean tick(double d)
    {
        if(decayByValue)    //Value
        {
            ...
            spanRatio = (float)map(value, startValue, endValue, 1, 0);
        }
        else    //Age
        {
            age += d;
    ...
    public Vec3 getPos()
    {
        return new Vec3
        (
            map(spanRatio, 1, 0, startPos.x(), endPos.x()),
            map(spanRatio, 1, 0, startPos.y(), endPos.y()),
            map(spanRatio, 1, 0, startPos.z(), endPos.z())
       );
    }
    ...
    {
        return map(spanRatio, 1, 0, startSize, endSize) * worldXpDropsSizeMultiplier * 0.02;
    }

    public float getRotation()
    {
        return (float) map(spanRatio, 1, 0, startRot, endRot);
    }

    public int getColor()
    {
        if(hueColor)
            return Util.hueToRGB((float) map(spanRatio, 1, 0, startHue, endHue) % 360, (float) Util.mapCapped(spanRatio, 0, 1, endSaturation, startSaturation), (float) Util.mapCapped(spanRatio, 0, 1, endBrightness, startBrightness));
        else
            return color;
    }
    ...
    private double map(double input, double inLow, double inHigh, double outLow, double outHigh){
        return (inLow == inHigh) ? outHigh : ((input - inLow) / (inHigh - inLow)) * (outHigh - outLow) + outLow;
    }*/
}
