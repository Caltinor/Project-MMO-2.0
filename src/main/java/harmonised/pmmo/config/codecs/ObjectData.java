...
public record ObjectData(
		...
		public ObjectData(boolean override) {
			this(override, new HashSet<>(), new HashMap<>(), new HashMap<>(), new HashMap<>(), new HashMap<>(),
			...
			BiConsumer<ObjectData, ObjectData> bothOrNeither = (o, t) -> {
				...
				t.bonuses().forEach((event, map) -> {
					bonuses.merge(event, map, (oMap, nMap) -> {
						...
						nMap.forEach((k, v) -> mergedMap.merge(k, v, Double::max));
						return mergedMap;
					});
				});
				reqs.putAll(o.reqs());	
				t.reqs().forEach((event, map) -> {
					reqs.merge(event, map, (oMap, nMap) -> {
						...
						nMap.forEach((k, v) -> mergedMap.merge(k, v, Integer::max));
						return mergedMap;
					});
				});
