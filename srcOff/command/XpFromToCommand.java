...
public class XpFromToCommand
{
    public static int execute(  ) throws CommandException
    {
        ...
        if( args.length > 4 )
        {
            ...
            player.sendStatusMessage( new TextComponentTranslation( "pmmo.xpFromTo", DP.dp(goalXp - xp), ( level % 1 == 0 ? (int) Math.floor( level ) : DP.dp(level) ), ( goalLevel % 1 == 0 ? (int) Math.floor( goalLevel ) : DP.dp(goalLevel) ) ), false );
        }
        else
            player.sendStatusMessage( new TextComponentTranslation( "pmmo.xpAtLevel", ( level % 1 == 0 ? (int) Math.floor( level ) : DP.dp(level) ), DP.dp(xp) ), false );
